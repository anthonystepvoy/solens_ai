// File: backend/js_scrapers/discover_wallets_top_24hr.js
// --- DISCOVER WALLETS FROM TOP 24HR TOKENS ---
...
async function main() {
  ...
  console.log("--- DISCOVERING WALLETS FROM TOP 24HR TOKENS ---");
  ...
  try {
    ...
    // 1. Get the LATEST 20 tokens from the daily_rank_snapshots collection.
    const tokensToProcess = await db.collection("daily_rank_snapshots")
      .find({})
      .sort({ retrieved_at: -1 })
      .limit(20)
      .toArray();
    ...
    if (tokensToProcess.length === 0) {
      console.log("✓ No tokens found in the daily_rank_snapshots collection. Exiting.");
      return;
    }
    ...
    for (const coin of topQualityTokens) {
      ...
      console.log(`\n--- Processing 24HR Token: ${coin.symbol || coinAddress.slice(0, 6)} (Score: ${coin.quality_score}/100) ---`);
      try {
          ...
          for (const trader of topTraders) {
            ...
            await db.collection("wallets").updateOne(
              ...
              { 
                ...
                $addToSet: { discovered_by: 'Top_24HR_Token_Discovery' } 
              },
              { upsert: true }
            );
    ...
    await browser.close();
    console.log("--- 24HR WALLET DISCOVERY COMPLETE ---");
  } catch (error) {
    console.error("✗ A fatal error occurred in the 24hr discovery:", error);
  } finally {
    await client.close();
    console.log("--- 24HR DISCOVERY SCRIPT FINISHED ---");
  }
}
